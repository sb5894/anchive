...
2. 명단 CSV 준비 (`scripts/roster.sample.csv` 형식 참고: `grade,class,number,name`). **실제 학생 이름이 든 CSV는 절대 커밋하지 않는다** —
   ...
   - **유치원**은 학년·반에 한글을 그대로 쓴다: `유치원,햇살반,3,김유치` → 화면에 "유치원 햇살반 김유치"
   - **초등**은 숫자를 쓴다: `3,2,15,이학생` → 화면에 "3-2 이학생"
   - **교사는 번호를 비우지 말고 `99`로 넣는다**: `3,2,99,최담임` → 이름 고르기에서 번호 없이 "최담임"으로 뜬다.
     번호를 비우면 같은 반 교사들의 문서 id가 겹쳐 한 명만 남는다(아래 검사에서 걸린다).

   `importRoster.mjs`는 업로드 전에 빈 칸과 **학년-반-번호 중복**을 검사해서, 문제가 있으면
   아무것도 쓰지 않고 몇 행이 겹쳤는지 알려준다. 명단이 조용히 사라지는 걸 막기 위한 것이므로
   에러가 나면 CSV를 고쳐서 다시 실행하면 된다.
3. 실행
   ```bash
...
명단의 `previousNumbers`는 번호 정정 전의 번호를 저장하는 이력이다. 앱은 학년·반·이름과
이전 번호가 모두 일치할 때 저장된 본인 번호를 정정하며, 익명 계정 UID는 유지한다.
`importRoster.mjs`는 같은 학생의 정정 이력을 보존하며, 정정 전 번호가 든 오래된 CSV는
업로드 전에 차단한다. 번호 정정 배포 후에는 사용 중인 화면을 새로고침해야 한다.
