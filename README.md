...
3. 실행
   ...
   명단만 다시 올릴 때는 **`scripts/명단올리기.bat`을 더블클릭**해도 된다(터미널을 열지 않아도 됨).
   `scripts/roster.csv`를 새로 저장해 두고 실행하면 되고, CSV나 서비스 계정 키가 없으면
   무엇이 없는지 알려주고 멈춘다. 덮어쓰기 전에 한 번 확인을 받는다.
